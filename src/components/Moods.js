...
export default function Moods() {
  /* STEP 1 statehook to set mood default to initialMood */
const [mood, setMood] = useState(initialMood)
  const makeHappy = () => {
    /* STEP 4 call the setmood function to set the const mood to be the const happyMood */
    setMood(happyMood)
  };
  const makeSad = () => {
    /* STEP 5 change mood to be sad*/
    setMood(sadMood)
  };
  const reset = () => {
    /* STEP 6 reset to initial */
    setMood(initialMood)
  };
  ...
  const style = {
    fontSize: '1.5em',
    marginBottom: '0.3em',
    color: mood === happyMood ? 'royalblue' : 'crimson', /* STEP 2 a key can return a simple ternerary */
  };

  return (
    <div className='widget-moods container'>
      <h2>Moods</h2>
      <div id='mood' style={style}>{mood}</div> {/* STEP 3 changed the hard coded text to be the const mood */}
      <div>
