...
const [celebrate, setCelebrate] = useState(null)
  ...
  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: celebrate ? 'gold' : 'royalblue', // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          list.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { /* in here set the featured id to be dev.id */setCelebrate(dev.id) }}>Feature</button>
            </div>
          )
        }
      </div>
